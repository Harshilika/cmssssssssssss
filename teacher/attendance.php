...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['attendance'])) {
    ...
    foreach ($_POST['attendance'] as $student_id => $status) {
        ...
        $check_sql = "SELECT * FROM attendance WHERE std_id = $student_id AND attendance_month = '$attendance_date' AND course_id = $course_id";
        $check_query = mysqli_query($db_conn, $check_sql);

        if (mysqli_num_rows($check_query) > 0) {
            // Update existing attendance record
            $update_sql = "UPDATE attendance SET attendance_value = '$status', modified_date = NOW() WHERE std_id = $student_id AND attendance_month = '$attendance_date' AND course_id = $course_id";
            mysqli_query($db_conn, $update_sql);
        } else {
            // Insert new attendance record
            $insert_sql = "INSERT INTO attendance (std_id, attendance_month, attendance_value, modified_date, course_id) VALUES ($student_id, '$attendance_date', '$status', NOW(), $course_id)";
...
// Fetch students based on class and course selection
$students = [];
$class_id = null;
$course_id = null;
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['class_id']) && !empty($_POST['course_id'])) {
    $class_id = $_POST['class_id'];
    $course_id = $_POST['course_id'];
                    ...
                    <tbody>
                        <?php foreach ($students as $student): ?>
                            <tr>
                                ...
                                <td>
                                    <input type="checkbox" name="attendance[<?php echo $student['id']; ?>]" value="Absent">
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
